...
int main() {
    int result = -1;
    char deviceId[] = "Caribou";
    char* registrationData = NULL;

    printf("\n*** Getting and introspecting an OAuth 2.0 token\n");
    printf("*** SDK function(s): iec_initialise, iec_device_register, iec_device_tokens, iec_json_*, iec_token_introspect\n\n");
    ...
    if( iec_set_attribute(IEC_ENDPOINT, "tcp://172.16.0.11:5556") < 0
        ...
        || iec_set_attribute(IEC_CLIENT_ID, "access-token-client") < 0
    ...
    printf("Requesting OAuth 2.0 access token for device (id: %s)... ", deviceId);
    char* tokens;
    ...
    printf("Extracting access token...\n");
    char* accessToken;
    if (iec_json_parse(tokens) != 0
        || iec_json_get_string("access_token", &accessToken)) {
        char* error = iec_last_error();
        printf("Failed to extract access tokens: %s\n", error);
        free(error);
        return -1;
    }
    ...
    printf("Introspecting access token...\n");
    char* introspection;
    result = iec_token_introspect(accessToken, &introspection);
    free(accessToken);
    if (result < 0) {
        char* error = iec_last_error();
        printf("Token introspection failed: %s\n", error);
        free(error);
        return result;
    }
